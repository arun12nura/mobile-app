import 'controller/ev_bike_issue_page_controller.dart';import 'package:dos/core/app_export.dart';import 'package:dos/widgets/custom_elevated_button.dart';import 'package:dos/widgets/custom_radio_button.dart';import 'package:dos/widgets/custom_text_form_field.dart';import 'package:flutter/material.dart';import 'package:dos/presentation/ev_bike_page_bottomsheet/ev_bike_page_bottomsheet.dart';import 'package:dos/presentation/ev_bike_page_bottomsheet/controller/ev_bike_page_controller.dart';
// ignore_for_file: must_be_immutable
class EvBikeIssuePageBottomsheet extends StatelessWidget {EvBikeIssuePageBottomsheet(this.controller, {Key? key}) : super(key: key);

EvBikeIssuePageController controller;
...
/// Section Widget
Widget _buildRadioGroup() { return Align(alignment: Alignment.centerLeft, child: Padding(padding: EdgeInsets.only(left: 32.h, right: 87.h), child: Obx(() => Row(children: [CustomRadioButton(text: "lbl_normal".tr, value: controller.evBikeIssuePageModelObj.value.radioList.value[0], groupValue: controller.radioGroup.value, padding: EdgeInsets.symmetric(vertical: 1.v), onChange: (value) {controller.radioGroup.value = value;}), Padding(padding: EdgeInsets.only(left: 78.h), child: CustomRadioButton(text: "lbl_ev".tr, value: controller.evBikeIssuePageModelObj.value.radioList.value[1], groupValue: controller.radioGroup.value, padding: EdgeInsets.symmetric(vertical: 1.v), onChange: (value) {controller.radioGroup.value = value;}))])))); } 
/// Section Widget
...
/// and the [EvBikePageBottomsheet] widget.
///
/// The bottom sheet is controlled by the [EvBikePageController]
/// and is displayed using the [Get.bottomSheet] method with
/// [isScrollControlled] set to true.

onTapSubmit() { Get.bottomSheet(EvBikePageBottomsheet(Get.put(EvBikePageController(),),),isScrollControlled: true,); } 
 }
